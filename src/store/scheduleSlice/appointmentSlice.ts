...
const API_BASE_URL = "/features/schedules/appointment";
...
export const fetchAppointments = createAsyncThunk(
  ...
  async ({ status }: { status?: AppointmentStatus } = {}, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.error || 'Error fetching appointments');
    }
  }
);
...
export const fetchAppointmentById = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.error || 'Error fetching appointment');
    }
  }
);
...
export const createAppointment = createAsyncThunk(
  ...
  async (appointmentData: Omit<Appointment, '_id' | 'userId' | 'createdAt' | 'updatedAt'>, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(API_BASE_URL, appointmentData);
      return response.data.appointment;
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.error || 'Error creating appointment');
    }
  }
);
...
export const updateAppointment = createAsyncThunk(
  ...
  async ({ id, data }: { id: string; data: Partial<Appointment> }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.error || 'Error updating appointment');
    }
  }
);
...
export const deleteAppointment = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.error || 'Error deleting appointment');
    }
  }
);
...
const appointmentsSlice = createSlice({
  ...
  reducers: {
    setSearchTerm: (state, action: PayloadAction<string>) => {
      ...
      state.filteredAppointments = state.appointments.filter(apt => 
        apt.title.toLowerCase().includes(action.payload.toLowerCase()) ||
        apt.location.toLowerCase().includes(action.payload.toLowerCase()) ||
        apt.status.toLowerCase().includes(action.payload.toLowerCase()) ||
        apt.privacy.toLowerCase().includes(action.payload.toLowerCase()) ||
        (apt.notes?.toLowerCase().includes(action.payload.toLowerCase()) ?? false) ||
...
export const selectPrivateAppointments = (state: { appointments: AppointmentsState }) =>
  state.appointments.filteredAppointments.filter(apt => apt.privacy === PrivacyType.PRIVATE);

export const selectUpcomingAppointments = (state: { appointments: AppointmentsState }) =>
  state.appointments.filteredAppointments
    .filter(apt => new Date(apt.dueDateTime) > new Date())
